...
from fyp_trading.transformer_cv import fixed_window_cv_transformer
from fyp_trading.transformer_models import TransformerConfig
...
def main() -> None:
    ...
    # A reasonable small Transformer baseline for your current feature sequences.
    ...
    metrics_df, preds_df, last_artifacts = fixed_window_cv_transformer(
        X_seq_all=X_seq,
        y_all=y_all,
        seq_index=seq_index,
        cfg_pipe=pipe_cfg,
    ...
    metrics_path = reports_dir / f"transformer_cv_metrics_{ts}.csv"
    preds_path = reports_dir / f"transformer_cv_preds_{ts}.csv"
    ...
    bt_ts_path = reports_dir / f"transformer_backtest_timeseries_{ts}.csv"
    bt_stats_path = reports_dir / f"transformer_backtest_stats_{ts}.json"
    bt_df.to_csv(bt_ts_path, index=False)
    save_json(bt_stats_path, bt_stats)
    print("Saved:", bt_ts_path.name, bt_stats_path.name)

    title = (
        f"Transformer Backtest ({pipe_cfg.ticker}) | thr={pipe_cfg.proba_threshold:.2f}, "
    ...
    fig_path = plots_dir / f"transformer_backtest_{ts}.png"
    save_figure(fig, fig_path)
    print("Saved:", fig_path.name)

    if last_artifacts is not None:
        model_path = models_dir / f"transformer_last_fold_{ts}.pt"
        torch.save(last_artifacts, model_path)
        print("Saved:", model_path.name)

    cfg_path = reports_dir / f"transformer_run_config_{ts}.json"
    save_json(
        cfg_path,
        {
            "seed": seed,
