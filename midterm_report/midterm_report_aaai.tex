...
\title{From LSTM Baselines to Transformers: A Midterm Milestone for a Quantitative Trading System}
...
\begin{abstract}
This midterm report presents the first end-to-end milestone of a quantitative algorithmic trading system prototype. Using 2800.HK daily OHLCV data over the most recent three years, we first established classic rule-based strategy baselines (RSI, Bollinger mean-reversion, dual moving-average trend, and Donchian breakouts) as an interpretability-first benchmark. We then implemented a complete ML workflow: data ingestion, technical feature engineering, volatility-adaptive 3-class labeling, fixed-window walk-forward cross-validation, and a cost-aware backtest driven by out-of-sample model probabilities. We report the results of two milestone runs (LSTM then Transformer), reflect on what we learned from turning predictions into tradable positions, and outline the next stage: scaling research beyond baselines while keeping time-respecting evaluation and reproducibility.
\end{abstract}

\section{Project Motivation and Midterm Goal}
Financial markets are complex, noisy, and impacted by many interacting factors. While the Efficient Market Hypothesis suggests persistent alpha is difficult, short-horizon patterns may exist and can be explored through careful modeling and robust backtesting. The long-term goal of this project is to design and evaluate a complete data-driven trading system. The midterm goal is more concrete: build a pipeline that is (i) time-respecting (no lookahead leakage), (ii) reproducible, and (iii) able to generate standardized artifacts (metrics, plots, logs) for iterative research.
...
\textbf{Time span.} Three years of daily bars were requested (period=3y, interval=1d). In the latest run, the effective date range was 2022-12-29 to 2025-12-29 with 733 daily observations.
...
We use 16 technical features derived from price history:
\begin{itemize}
...
\end{itemize}
For this exploration phase, we report results over the full 3-year sample (733 daily observations). Later, ML/DL runs use a ``last 252 trading days'' backtest window to match the out-of-sample walk-forward prediction horizon.

\subsection{Representative strategies}
\textbf{Mean-reversion (RSI / Bollinger).} These strategies attempt to buy short-term weakness and sell strength. In our run, RSI achieved a high Sharpe ratio but extremely low coverage (only trading on a small fraction of days), which naturally limits total return compared to Buy\&Hold in a generally rising market.

\textbf{Trend-following (Dual MA / Donchian).} Trend strategies aim to capture persistent moves but can suffer in choppy regimes through whipsaws. In our run, Dual MA had much higher coverage (near half the days) but also higher drawdowns, while Donchian breakouts underperformed Buy\&Hold over this 3-year window.
...
We convert the feature table into fixed-length sequences with lookback=30. Each training sample is a tensor of shape $[30,16]$ and is labeled at the end of the window.

\subsection{Architecture and training}
We train a PyTorch LSTM multi-class classifier with the following configuration:
\begin{itemize}
...
\end{itemize}
\textbf{Masked loss.} To emphasize directional decisions, we compute cross-entropy loss on Up/Down samples only; Neutral samples are excluded from the loss (but still appear as a prediction class).

\subsection{Fixed-window walk-forward cross-validation}
We use a time-respecting fixed-window scheme:
\begin{itemize}
...
For each out-of-sample day, the model outputs probabilities $(P(\text{Down}),P(\text{Neutral}),P(\text{Up}))$. We apply a confidence threshold of 0.57:
\begin{itemize}
...
\end{itemize}
We additionally enforce a minimum holding period of 2 trading days to reduce rapid flip-flopping.

\subsection{Returns, costs, and equity curves}
We convert log returns to simple returns: $R_t=\exp(r_t)-1$. Strategy return is $pos_t\cdot R_t$. Transaction cost is charged on absolute position changes at 2 basis points per change. We compute equity curves by cumulative multiplication.
...
To explore time-series Transformers designed for efficiency on longer sequences, we implemented an Informer-style encoder classifier inspired by Informer \citep{informer2021}. Since our task is classification (Up/Neutral/Down) rather than multi-step forecasting, we adapt Informer as an \emph{encoder-only} model with:
\begin{itemize}
...
\end{itemize}
We keep the same evaluation protocol as the Transformer run (same data span, features, labels, walk-forward CV, confidence threshold, holding period, and transaction costs).
...
Figure~\ref{fig:inf_bt} shows the Informer-style backtest visualization. Compared to Buy\&Hold, the strategy remains close to flat over this 1-year window and underperforms in total return, despite moderate exposure (coverage) and relatively low turnover.
...
\begin{itemize}
  \item \textbf{Reproducibility matters.} A research loop becomes faster once each run produces standardized artifacts: run configuration, per-fold metrics, per-day predictions, backtest statistics, and plots.
  ...
  \item \textbf{Trading logic is part of the model.} The confidence threshold, minimum holding period, and cost model directly shape the realized strategy behavior; these assumptions must be explicit and consistent across model comparisons.
\end{itemize}
These lessons shape how we will evaluate the next generation of models.

\section{Next Stage: Beyond Baselines (Transformers, Market-Aware Models, and LLM Signals)}
With both an LSTM baseline and a Transformer baseline implemented under a consistent evaluation protocol, the next stage is to scale model capacity and research scope while preserving methodological rigor. Our plan is to:
\begin{itemize}
